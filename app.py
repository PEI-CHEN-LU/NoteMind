...
class ContentItem(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    # 新增 relationship 到 Topic（如果還沒有的話）
    # topic = db.relationship('Topic', backref=db.backref('content_items', lazy=True))
...
def save_notes(topic_id):
    ...
    try:
        ...
        new_content = ContentItem(
            title="AI 回答筆記",
            content_type="note",  # 可為 'note', 'file', 'link'
            content=notes_content,
            topic_id=topic.id,  # 替換成你要綁定的 topic ID
            user_id=current_user.id
        )
